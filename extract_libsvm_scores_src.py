...
""" Extracts scores from libsvm's output files, grouped by source.
    Ie, only the output files predicted from the specified source model are
    processed.
"""
...
def main():
  ...
  parser.add_argument('--src_date', required=True)
  parser.add_argument('--output_dir', required=True)
  args = parser.parse_args()

  # Eg, 2013-06-30.txt.scale_2013-04-21.out
  prog = re.compile('^\d{4}-\d{2}-\d{2}\.txt\.scale_%s\.out$' % args.src_date)
